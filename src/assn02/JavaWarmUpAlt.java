...
public class JavaWarmUpAlt {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            List<String> list = Arrays.asList(categoriesList);
            catIndex = list.indexOf(category[i]);
    ...
    static int getMaxPriceIndex(double[] priceT){
        ...
        for (int i=0; i<priceT.length; i++) {
            if (priceT[i] > priceT[ret]) {
                ret = i;
            }
        }
        return(ret); // modify this as well
        // Your code ends here.
    }
